...
def cline_payload(model: str) -> dict:
    return {
        ...
        "customInstructions": (
            ...
            "(ingest this path, map this folder, list SQL tables, read memory index). "
            "If the user names a local folder or file path (C:\\..., \\\\server\\share, "
            "or file://), do not ask what they want with it. Immediately run "
            ".venv\\Scripts\\python.exe learn\\ingest_path.py --path \"<that path>\", "
            "then read memory/INDEX.md and the new data-maps and use them. That folder "
            "stays connected for later sessions. Without being asked: read "
            "memory/INDEX.md, WHAT_WORKED.md, FAILED.md, and data-maps before "
