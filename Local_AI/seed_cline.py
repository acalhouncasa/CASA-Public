...
def cline_payload(model: str) -> dict:
    return {
        ...
        "customInstructions": (
            ...
            "off the box. "
            "CRITICAL: If the user gives a local path (C:\\..., D:\\..., \\\\server\\share, "
            "file://) or says here is the location of data/CSVs, that IS the task. "
            "Do not ask what they want. Do not list options. Do not invent filenames "
            "(never invent 10e11_data.csv). Use the EXACT path they typed. If it is a "
            "folder of many CSVs, that is normal. FIRST action: run terminal command "
            ".venv\\Scripts\\python.exe learn\\ingest_path.py --path \"<exact path>\", "
            "wait for it, then read memory/INDEX.md and memory/data-maps/ and summarize "
            "what was mapped. That folder stays connected for later sessions. "
            "Starters: ingest this path, map this folder, list SQL tables, read memory. "
            "Without being asked: read memory/INDEX.md, WHAT_WORKED.md, FAILED.md, and "
            "data-maps before repeating work; after each task write a data map and a "
            "short lesson."
        ),
